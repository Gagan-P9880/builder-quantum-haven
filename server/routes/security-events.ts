...
const generateMockEvent = (): SecurityEvent => {
  ...
  if (type === "rfid") {
    ...
    return {
      ...
      description: isAuthorized ? "Valid access card detected" : "Unauthorized access attempt"
    };
  } else {
    ...
    return {
      ...
      description: isBlocked ? "DoS attack blocked by firewall" : "DoS attack detected - monitoring"
    };
  }
};
...
export const addSecurityEvent: RequestHandler = (req, res) => {
  ...
  if (!event.type || !event.status) {
    return res.status(400).json({ error: "Missing required fields: type, status" });
  }
